...
from nerfstudio.cameras.camera_paths import (
    ...
    get_origin_camera_path,
    get_path_from_json,
    get_spiral_path,
    get_interpolated_camera_path_usepose
)
...
def _render_trajectory_video(
    ...
    with ExitStack() as stack:
        ...
        # random smaple a style code z  # lxy  # 2023-05-20 change3
        if pipeline.model.config.use_style:
            sample_style = torch.randn((1, 64)).to(pipeline.device)
        ...
        with progress:
            for camera_idx in progress.track(range(cameras.size), description=""):
                ...
                if pipeline.model.config.use_style:  # lxy  # 2023-05-20 change3
                    camera_ray_bundle.metadata['styles'] = sample_style  
                ...
                if crop_data is not None:
                    ...
                    with torch.no_grad():
                        outputs = pipeline.model.get_outputs_for_camera_ray_bundle(camera_ray_bundle)  # mark render
...
class RenderTrajectory:
    ...
    def main(self) -> None:
        ...
        _, pipeline, _, _ = eval_setup(
            ...
            test_mode="test" if self.traj in ["spiral", "interpolate", "train"] else "inference",
        )
        ...
        if self.traj == "spiral":  # mark           
            ...
            camera_type = CameraType.PERSPECTIVE
            camera_path = get_origin_camera_path(
                cameras=pipeline.datamanager.train_dataset.cameras, steps=self.interpolation_steps
            )
