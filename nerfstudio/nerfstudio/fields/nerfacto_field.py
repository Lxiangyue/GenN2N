...
class StyleMLP_Wild_multilayers(nn.Module):  # lxy borrow from StylizedNeRF
...
class TCNNNerfactoField(Field):
    ...
    def __init__(
        ...
        # transients
        if self.use_transient_embedding:   # False
        ...
        # predicted normals
        if self.use_pred_normals:  # False
        ...
        self.mlp_style_head = tcnn.Network(
            n_input_dims=self.direction_encoding.n_output_dims + self.geo_feat_dim + self.appearance_embedding_dim + self.style_embedding_dim,
            n_output_dims=3,
            network_config={
                "otype": "FullyFusedMLP",
                "activation": "ReLU",
        ...
        # self.mlp_style_density = nn.Linear(16+64, 16)
        # self.mlp_style_density_residual = nn.Sequential(nn.Linear(64, 32), 
        #                                                 nn.ReLU(True),
        #                                                 nn.Linear(32, 16), 
        #                                                 nn.ReLU(True),
        #                                                 nn.Linear(16, 3),)  # -0.1, 0.07
        zero_linear = self.zero_linear
        self.mlp_style_density = zero_linear(16+64, 16)
        self.mlp_style_density_residual = nn.Sequential(zero_linear(64, 32), 
                                                        nn.ReLU(True),
                                                        zero_linear(32, 16), 
                                                        nn.ReLU(True),
                                                        zero_linear(16, 3),)  # -0.1, 0.07

        # # fix origin nerf parameters  # will encoding need to be fixed ???
    ...
    def zero_linear(self,in_dim,out_dim): # initial linear parameters to zero 
    ...
    def get_density(self, ray_samples: RaySamples) -> Tuple[TensorType, TensorType]:
        ...
        if 'use_deltaxyz' in ray_samples.metadata.keys():   # 2023-05-21  # False
            use_deltaxyz = bool(ray_samples.metadata['use_deltaxyz'][0,0,0])  # True or False
            if (use_deltaxyz) and ('styles' in ray_samples.metadata.keys()):
                delta_xyz = self.mlp_style_density_residual(ray_samples.metadata['styles'])  
                # delta_xyz = delta_xyz.clip(-0.01, 0.01)   # 0.9， 0.1 no
                positions = positions.clone() + delta_xyz  # [16384, 48, 3]
            
        # if 'styles' in ray_samples.metadata.keys():
        ...
        positions_flat = positions.view(-1, 3)   # xyz             
        
        h = self.mlp_base(positions_flat).view(*ray_samples.frustums.shape, -1)  # [16384, 48, 16]            
        
        if 'use_deltaxyz' in ray_samples.metadata.keys():   # 2023-05-21  # False
            use_deltaxyz = bool(ray_samples.metadata['use_deltaxyz'][0,0,0])  # True or False
            if (use_deltaxyz is False) and ('styles' in ray_samples.metadata.keys()):
                style_h = torch.cat(
                    [
                        h,
                        ray_samples.metadata['styles'].view(*ray_samples.frustums.shape, -1)
                    ],
                    dim=-1,
                )
                h = self.mlp_style_density(style_h)
            
        
        density_before_activation, base_mlp_out = torch.split(h, [1, self.geo_feat_dim], dim=-1) # [16384, 48, 1], [16384, 48, 15]
    ...
    def get_outputs(
        ...
        outputs_shape = ray_samples.frustums.directions.shape[:-1]  # [32768, 48, 3][:-1]
        ...
        if 'styles' in ray_samples.metadata.keys():  # 2023-5-14  # True  # before 2023-9-11 is False! bug
            h = torch.cat(
                [
                    h,
                    ray_samples.metadata['styles'].view(-1, self.style_embedding_dim),
                ],
                dim=-1,
            )  # 16 + 15 + 32 + 64 = 63 + 64 # lxy 64是否过大? # 2023-5-14
            rgb = self.mlp_style_head(h).view(*outputs_shape, -1).to(directions)  
        else:
            rgb = self.mlp_head(h).view(*outputs_shape, -1).to(directions)
