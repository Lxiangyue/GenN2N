...
from torch.utils.tensorboard import SummaryWriter  # 2023-05-21-td
...
class Trainer:
    ...
    def __init__(self, config: TrainerConfig, local_rank: int = 0, world_size: int = 1) -> None:
        ...
        # self.load_editimgs = True 
        # if self.load_editimgs:
        #     # load all ore edited images  # 2023-05-21  # lxy
        ...
        #     for i,path in enumerate(sorted(glob.glob('Data/face_imageDatasets_clown/face*'))):  # style
        #         for j,name in enumerate(sorted(glob.glob('%s/frame_*.jpg'%path))):              # view
        #             pil_image = Image.open(name) 
        #             image = np.array(pil_image, dtype="uint8")  # (384, 512, 3)
        #             pre_edited_imgs_one_style.append(torch.Tensor(image))
        #         pre_edited_imgs_one_style_stack = torch.stack(pre_edited_imgs_one_style).to(self.device)  # [65, 384, 512, 3]  # if on cpu, Killed
        #         pre_edited_imgs_one_style = []
        ...
        #     self.pre_edited_imgs = pre_edited_imgs  # [60, 65, 384, 512, 3]
        #     del pre_edited_imgs
    ...
    def train(self) -> None:
        ...
        if self.local_rank == 0:  # 2023-05-21-td
            # log = SummaryWriter('%s/%s'%(self.checkpoint_dir,'exp_name'), comment='exp_name')
            log = SummaryWriter('%s/'%self.checkpoint_dir)
        else: log=None
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                with self.train_lock:
                    with TimeWriter(writer, EventName.ITER_TRAIN_TIME, step=step) as train_t:
                        ...
                        loss, loss_dict, metrics_dict = self.train_iteration(step, self.checkpoint_dir)   # 20231125
                        
                        if self.local_rank==0:    # 2023-05-21-tb
    ...
    def _load_checkpoint(self) -> None:    
        ...
        if load_dir is not None:
            ...
            if load_step is None:
                ...
                new_lst = [item for item in os.listdir(load_dir) if 'event' not in item]
                load_step = sorted(int(x[x.find("-") + 1 : x.find(".")]) for x in new_lst)[-1]
            
            try:
                load_path: Path = load_dir / f"step-{load_step:09d}.ckpt"
                assert load_path.exists(), f"Checkpoint {load_path} does not exist"
            except:
                load_path: Path = load_dir / f"{load_step:09d}.ckpt"
                assert load_path.exists(), f"Checkpoint {load_path} does not exist"
    ...
    def train_iteration(self, step: int, checkpoint_dir=None) -> TRAIN_INTERATION_OUTPUT:    # 20231125
        ...
        with torch.autocast(device_type=cpu_or_cuda_str, enabled=self.mixed_precision):
            _, loss_dict, metrics_dict = self.pipeline.get_train_loss_dict(step=step, checkpoint_dir=checkpoint_dir, max_num_iterations=self.config.max_num_iterations, load_dir=self.config.load_dir)   # 20231125
