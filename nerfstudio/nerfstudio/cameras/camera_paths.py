...
# lxy
def get_interpolated_camera_path_usepose(cameras: Cameras, steps: int, poses_use) -> Cameras:
    ...
    cameras = Cameras(fx=Ks[:, 0, 0], fy=Ks[:, 1, 1], cx=Ks[0, 0, 2], cy=Ks[0, 1, 2], camera_to_worlds=poses_use.cpu())
    return cameras
...
def get_origin_camera_path(cameras: Cameras, steps: int) -> Cameras:
    """Generate a camera path between two cameras.
    ...
    Returns:
        A new set of cameras along a path.
    """
    Ks = cameras.get_intrinsics_matrices().cpu()
    poses = cameras.camera_to_worlds.cpu()
    # poses, Ks = get_interpolated_poses_many(poses, Ks, steps_per_transition=steps)
