...
class Model(nn.Module):
    ...
    # 2023-05-24
    # @torch.no_grad() 
    def get_outputs_for_camera_ray_bundle(self, camera_ray_bundle: RayBundle, return_depth_grad=False, return_rgb_grad=False) -> Dict[str, torch.Tensor]:  # mark
        ...
        if return_rgb_grad:
            num_rays_per_chunk /= 2
            num_rays_per_chunk = int(num_rays_per_chunk)
        ...
        for i in range(0, num_rays, num_rays_per_chunk):
            ...
            for output_name, output in outputs.items():  # type: ignore
                ...
                if return_depth_grad:
                    if (output_name != 'depth') and (output_name != 'rgb'):
                        outputs_lists[output_name].append(output.clone().detach())
                    else:
                        # import pdb; pdb.set_trace()
                        outputs_lists[output_name].append(output)  # only depth has grad
                    del output
                elif return_rgb_grad:  # 20230608
                    if output_name != 'rgb':
                        outputs_lists[output_name].append(output.clone().detach())
                    else:
                        import pdb; pdb.set_trace()
                    ...
                    outputs_lists[output_name].append(output.clone().detach())  # no grad

                # # all has grad  # 2023-09-10
