...
class InstructPix2Pix(nn.Module):
    ...
    def __init__(self, device: Union[torch.device, str], num_train_timesteps: int = 1000, ip2p_use_full_precision=False) -> None:
        ...
        # try:
        #     pipe = StableDiffusionInstructPix2PixPipeline.from_pretrained('/root/.cache/huggingface/instruct-pix2pix', torch_dtype=torch.float16, safety_checker=None)
        #     pipe.scheduler = DDIMScheduler.from_pretrained('/root/.cache/huggingface/stable-diffusion-v1-4', subfolder="scheduler")
        # except:
        #     pipe = StableDiffusionInstructPix2PixPipeline.from_pretrained('/apdcephfs_cq10/share_1330077/emmafxliu/Codes_GenN2N/huggingface/instruct-pix2pix', torch_dtype=torch.float16, safety_checker=None)
        #     pipe.scheduler = DDIMScheduler.from_pretrained('/apdcephfs_cq10/share_1330077/emmafxliu/Codes_GenN2N/huggingface/stable-diffusion-v1-4', subfolder="scheduler")
        ...
        n_unit=4
        unit_modules=[bottleneck_IR(min(2**(i+2),64),min(2**(i+3),64),2).to(self.device) for i in range(n_unit)]
        self.style_body = nn.Sequential(nn.Sequential(*unit_modules),torch.nn.AdaptiveAvgPool2d((1, 1)))
    ...
    def latent_to_styles(self, latents: TensorType["BS", 4, "H", "W"]) -> TensorType["BS", 64]:  # 2023-05-23
        """Convert latenst to styles 
        Args:
            Latents:
        Returns:
            Styles
        """
        bs=latents.shape[0]
        styles = self.style_body(latents).view(bs,-1)
        # styles = self.style_linear(styles)
